...
def test_join_geometries():
    ...
    assert (geometry.triangles == expected_tri).all()
    assert len(geometry.vertices) == 8


def test_geometry_normal_and_area():
    ...
    np.testing.assert_almost_equal(triangle_points, [[vp[0], vp[1], vp[2]], [vp[0], vp[2], vp[1]]])
    np.testing.assert_almost_equal(normals, [(0, 0, 1), (0, 0, -1)])
    np.testing.assert_almost_equal(areas, [0.5, 0.5])


def test_geometry_save_stl():
...
def test_geometry_filter_triangles_simple():
    ...
    for t_idx, t in enumerate(geometry.triangles):
        ...
        for v_idx in t:
            ...
            if (v >= start).all() and (v <= end).all():
                assert triangles_filtered[t_idx]
                is_in = True
                break
        if not is_in:
            assert not triangles_filtered[t_idx]


def test_geometry_filter_triangles_cylinder():
